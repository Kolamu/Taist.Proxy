...
public class ServerCertifcate {
    ...
    public byte[] decrypt(byte data[],SecureRandom seed) throws Exception{  
        // 对数据解密  
        Cipher cipher = Cipher.getInstance(privateKey.getAlgorithm());  
    ...
    public byte[] decryptByPublicKey(byte data[],SecureRandom seed) throws Exception{  
        if(publicKey==null){  
            publicKey=CertFactory.readPublicKeys();  
        }  
        // 对数据解密  
    ...
    public byte[] cactHash(byte[] bytes) {
        byte[] _bytes = null;  
        try {  
